...
public class GameView {
    ...
    private static class Version {
        private final static String VERSION = "2020.3";
        private final static String VERSION_SHORT = VERSION.substring(0, 6);
        private final static LocalDate DATE = LocalDate.parse("2020-11-21");
    ...
    private void initColormap() {
        ...
        standardColorMap.put('r', Color.RED.darker());
        standardColorMap.put('G', Color.GREEN);
        standardColorMap.put('g', Color.GREEN.darker());
        standardColorMap.put('B', Color.BLUE);
        standardColorMap.put('b', Color.BLUE.darker());
        standardColorMap.put('Y', Color.YELLOW);
        standardColorMap.put('y', Color.YELLOW.darker());
        standardColorMap.put('P', Color.PINK);
        standardColorMap.put('p', Color.PINK.darker());
        standardColorMap.put('C', Color.CYAN);
        standardColorMap.put('c', Color.CYAN.darker());
        standardColorMap.put('M', Color.MAGENTA);
        standardColorMap.put('m', Color.MAGENTA.darker());
        standardColorMap.put('O', Color.ORANGE);
        standardColorMap.put('o', Color.ORANGE.darker());
     ...
     */
    public int showStartScreen(String title, String description, String selectionTitle, String[] selectionItems,
                               int selectedItem) {
        StartScreen startScreen = new StartScreen(this, title, description, selectionTitle, selectionItems,
                selectedItem);
        startScreen.printStartScreen();
        return startScreen.getSelectedItem();
    }
     ...
     */
    public void addPolygonToCanvas(int[] xCoordinates, int[] yCoordinates, int lineWeight, boolean filled,
                                   Color color) {
     ...
     */
    public void addPolyLineToCanvas(int[] xCoordinates, int[] yCoordinates, int lineWeight, Color color) {
        canvas.addPolyLineToCanvas(xCoordinates, yCoordinates, lineWeight, color);
    }
     ...
     * gedrückten Tasten als <code>KeyCode</code> der Klasse {@link KeyEvent} als Array zurück. Die Tasten sind in der
     * Reihenfolge enthalten, in der sie gedrückt wurden. Diese Methode ist geeignet um die Steuerung von Spielfiguren
     * zu realisieren.
     * <p>
     ...
     * (Canvas) und die Information ob die Maus gedrückt, losgelassen, gecklickt oder nur bewegt wurde. Um
     * festzustellen, wie die Maus betätigt wurde, kann der Typ des Ereignises mit {@link MouseEvent#getID()} abgefragt
     ...
     * MouseEvent#getY()} = Y-Koordiante<br> abgerufen werden, um X und Y-Koordinate des Ereignisses zu bestimmen.<br>
     * <br>
    ...
    private static class SwingAdapter {
        ...
        void setWindowIcon(String windowIcon) {
            ...
            try {
                fensterSymbol = new ImageIcon(GameView.class.getResource("/ressources/" + windowIcon)).getImage();
            } catch (Exception e) {
                e.printStackTrace();
        ...
        void setMouseCursor(String cursor, boolean centered) {
            try {
                Image im = new ImageIcon(GameView.class.getResource("/ressources/" + cursor)).getImage();
    ...
    private static class StartScreen {
        ...
        StartScreen(GameView gameView, String title, String description, String selectionTitle,
            ...
            this.descriptionFontSize = (int) Math.rint(titleFontSize * 0.35);
            this.yDescription = titleHeight + 2 * lineWeight;

            int gap = 20;
            int selectionFontSize = (int) Math.rint(titleFontSize * 0.35);
            ...
            this.selectionBox = new SelectionBox(gameView, selectionTitle, selectionItems, selectedItem,
                    selectionFontSize, selectionBoxLineWeight, font, Color.YELLOW, Color.BLACK, frameAndTitle);
        ...
        private void addRectangles() {
            gameView.addRectangleToCanvas(lineWeight / 2, lineWeight / 2, WIDTH - 1 - lineWeight,
                    HEIGHT - 1 - lineWeight, lineWeight, false, font);
            gameView.addRectangleToCanvas(lineWeight / 2, lineWeight / 2, WIDTH - 1 - lineWeight,
                    titleHeight - lineWeight, lineWeight, false, font);
            gameView.addRectangleToCanvas(lineWeight / 2, yLowerLine + lineWeight / 2, WIDTH - 1 - lineWeight,
                    HEIGHT - yLowerLine - lineWeight, lineWeight, false, font);
        }

        private void addTitle() {
            gameView.addTextToCanvas(title, (WIDTH - title.length() * titleFontSize) / 2,
                    ((int) (titleFontSize * 1.5) - titleFontSize) / 2, titleFontSize, frameAndTitle);
        }

        private void addDescription() {
            gameView.addTextToCanvas(description, 2 * lineWeight, yDescription, descriptionFontSize, font);
        }
        ...
        private void addEnterField() {
            ...
            gameView.addTextToCanvas(text, enterBox.x + (enterBoxWidth - titleWidth) / 2,
                    enterBox.y + (enterBoxHeight - titleHeight) / 2, descriptionFontSize, frameAndTitle);
        }
        ...
        private static class SelectionBox {
            ...
            private void addTitleBox() {
                gameView.addRectangleToCanvas(x, y, width - lineWeight, titleHeight - lineWeight, lineWeight, false,
                        frameAndTitle);
                gameView.addTextToCanvas(title, xLine + (widthOfMarkerField - title.length() * fontSize) / 2,
                        y + (titleHeight - fontSize) / 2, fontSize, frameAndTitle);
            }
            ...
            private void addMarkerField(java.awt.Rectangle bounds, String name, boolean isMarked) {
                if (isMarked) {
                    gameView.addRectangleToCanvas(bounds.x + lineWeight / 2, bounds.y + lineWeight / 2,
                            bounds.width - lineWeight, bounds.height - lineWeight, lineWeight, true, markerHighlight);
                    gameView.addRectangleToCanvas(bounds.x + lineWeight / 2, bounds.y + lineWeight / 2,
                            bounds.width - lineWeight, bounds.height - lineWeight, 1, false, markerRectangle);
                } else {
                    gameView.addRectangleToCanvas(bounds.x, bounds.y, bounds.width, bounds.height, 1, false,
                            markerFont);
                }
                gameView.addTextToCanvas(name, bounds.x + lineWeight, bounds.y + (bounds.height - fontSize) / 2,
                        fontSize, markerFont);
            }
            ...
            private void addNavigationBox() {
                ...
                gameView.addRectangleToCanvas(xUpDown, yMarkerBox, 4 * lineWeight,
                        items.length * heightOfMarkerField + lineWeight, lineWeight, false, frameAndTitle);
                gameView.addPolygonToCanvas(new int[]{xUpDown + lineWeight, xUpDown + 3 * lineWeight,
                        xUpDown + 2 * lineWeight}, new int[]{yUp, yUp, yUp - fontSize / 2}, 1, true, frameAndTitle);
                gameView.addPolygonToCanvas(new int[]{xUpDown + lineWeight, xUpDown + 3 * lineWeight,
                        xUpDown + 2 * lineWeight}, new int[]{yDown, yDown, yDown + fontSize / 2}, 1, true,
                        frameAndTitle);
            }
